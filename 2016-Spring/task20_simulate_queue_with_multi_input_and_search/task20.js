...
var btns = document.getElementsByClassName("panel")[0].get
...
function isInputNotEmpty(rawInput) {
    // rawInput 可以为：数字、中文、英文等
    // rawInput 的分隔符可以是：逗号（全角半角均可），顿号，回车，空格（全角半角、Tab等均可）等符号
    var data = rawInput.split(/,|，|、|\u3000|\s/).filter(function(elem) {
        if (elem !== "") {
            return true;
        }
    });
...
function validInput(rawInput) {
    return isInputNotEmpty(rawInput);
}
...
var queue = (function() {
    ...
    return {
        ...
        leftIn: function(value) {
            ...
            data = validInput(value).concat(data);
            drawQueue();
            return this;
        },
        rightIn: function(value) {
            ...
            data = data.concat(validInput(value));
            drawQueue();
            return this;
        },
        leftOut: function() {
